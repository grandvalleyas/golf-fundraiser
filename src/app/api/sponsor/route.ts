...
export async function POST(request: Request) {
	const { userId, name, tier, logo, text, websiteLink, freeGolfers } =
		await request.json();

	if (!userId || !name || !tier) {
		return NextResponse.json(
			{ error: "User ID, name, and tier are required" },
			{ status: 400 }
		);
	}
	...
	if (!logo && !text) {
		return NextResponse.json(
			{ error: "At least one of logo or text must be provided" },
			{ status: 400 }
		);
	}
	...
	try {
		...
		if (existingSponsor) {
			return NextResponse.json(
				{ error: "User already has a sponsor" },
				{ status: 400 }
			);
		}
...
export async function PUT(request: Request) {
	const { userId, name, logo, text, websiteLink, freeGolfers } =
		await request.json();
	...
	try {
		...
		if (freeGolfers) updateData.freeGolfers = freeGolfers;

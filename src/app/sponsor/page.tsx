...
const sponsorTiers: SponsorTier[] = [
  ...
  },
  {
    ...
    price: 1000,
    freeGolfers: 1,
    description: "Includes one golfer, recognition on all carts, hole sponsor sign",
  },
  {
  ...
  },
  {
    ...
    price: 3000,
    freeGolfers: 4,
    description: "Includes foursome, recognition at dinner, hole sponsor sign",
  },
  {
    name: "Title Sponsor",
    price: 10000,
    freeGolfers: 4,
    description: "Includes foursome, outing recognition, hole sponsor sign, sponsor gift",
  },
];
...
export default function Sponsor() {
    ...
    if (success && sessionId) {
      const handlePaymentSuccess = async () => {
        try {
          ...
          const method = isUpgrade === "true" ? "PUT" : "POST";
          ...
          if (!createResponse.ok) {
            ...
            throw new Error(error.error || "Failed to process sponsorship");
          }

          toast({
            title: "Success",
            description: isUpgrade === "true" ? "Sponsorship upgraded successfully!" : "Sponsorship created successfully!",
          });
          fetchSponsor();
          setName("");
          ...
          toast({
            title: "Error",
            description: (err as Error).message || "Failed to process sponsorship",
            variant: "destructive",
          });
        }
      };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: (err as Error).message || "Failed to initiate payment",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const handleEditSponsorChange = (field: keyof Sponsor, value: any) => {
  ...
  const handleSubmitEditSponsor = async () => {
    if (!editSponsor || !editSponsor.name || !editTier) {
      toast({
        title: "Error",
        description: "Name and tier are required",
        variant: "destructive",
    ...
    try {
      ...
      const selectedTier = sponsorTiers.find((t) => t.name === editTier);
      if (!selectedTier) throw new Error("Invalid tier selected");

      const originalPrice = sponsor ? sponsor.price : 0;
      const newPrice = selectedTier.price;
      const additionalAmount = Math.max(0, newPrice - originalPrice);

      if (additionalAmount > 0) {
        // Tier upgrade requires payment
        const response = await fetch("/api/sponsor/create-checkout-session", {
          method: "POST",
          body: JSON.stringify({
        ...
        if (!response.ok) {
          const error = await response.json();
          throw new Error(error.error || "Failed to create checkout session for upgrade");
        }
      ...
      toast({
        title: "Error",
        description: (err as Error).message || "Failed to update sponsor",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const selectedTier = sponsorTiers.find((t) => t.name === tier);
  const selectedEditTier = sponsorTiers.find((t) => t.name === editTier);
          ...
          {sponsor ? (
              ...
              <CardContent className="space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm sm:text-base">
                      <strong>Tier:</strong> {sponsor.tier}
                    </p>
                    <p className="text-sm sm:text-base">
                      <strong>Price Paid:</strong> ${sponsor.price.toLocaleString()}
                    </p>
                    <p className="text-sm sm:text-base">
                      <strong>Free Golfers:</strong> {sponsor.freeGolfers.length > 0 ? sponsor.freeGolfers.join(", ") : "None"}
                    </p>
                    {sponsor.websiteLink && (
                      <p className="text-sm sm:text-base">
                ...
                <Dialog
                  ...
                  <DialogContent className="sm:max-w-[600px]">
                    <DialogHeader>
                      <DialogTitle className="text-lg sm:text-xl">Edit Sponsorship</DialogTitle>
                    </DialogHeader>
                    <div className="space-y-4">
                      ...
                      <div>
                        <Label htmlFor="edit-tier" className="text-sm sm:text-base">
                          Sponsorship Tier
                        </Label>
                        <Select value={editTier} onValueChange={setEditTier}>
                          <SelectTrigger id="edit-tier" className="w-full text-sm sm:text-base mt-1">
                      ...
                      {selectedEditTier && selectedEditTier.freeGolfers > 0 && (
                        <div>
                          <Label className="text-sm sm:text-base">Free Golfers ({selectedEditTier.freeGolfers})</Label>
                          {Array.from({ length: selectedEditTier.freeGolfers }).map((_, index) => (
                            <Input
                              key={index}
                              placeholder={`Golfer ${index + 1} name`}
                              value={editSponsor?.freeGolfers[index] || ""}
                              onChange={(e) => {
                                const newGolfers = [...(editSponsor?.freeGolfers || [])];
                                newGolfers[index] = e.target.value;
                                handleEditSponsorChange("freeGolfers", newGolfers);
                              }}
                      ...
                      <Button onClick={handleSubmitEditSponsor} className="w-full bg-green-500 hover:bg-green-600 text-sm sm:text-base py-2" disabled={loading}>
            ...
            <Card>
              ...
              <CardContent className="space-y-4 grid grid-cols-1 gap-2 sm:gap-4">
                ...
                <div>
                  <Label htmlFor="tier" className="text-sm sm:text-base">
                    Sponsorship Tier
                  </Label>
                  <Select value={tier} onValueChange={setTier}>
                    <SelectTrigger id="tier" className="w-full text-sm sm:text-base mt-1">
                      <SelectValue placeholder="Select a tier" />
                    </SelectTrigger>
                    <SelectContent>
                      {sponsorTiers.map((tier) => (
                ...
                {selectedTier && selectedTier.freeGolfers > 0 && (
                  <div>
                    <Label className="text-sm sm:text-base">Free Golfers ({selectedTier.freeGolfers})</Label>
                    {Array.from({ length: selectedTier.freeGolfers }).map((_, index) => (
                      <Input
                        key={index}
                        placeholder={`Golfer ${index + 1} name`}
                        value={freeGolfers[index] || ""}
                        onChange={(e) => {
                          const newGolfers = [...freeGolfers];
                          newGolfers[index] = e.target.value;
                          setFreeGolfers(newGolfers);
                        }}
                        className="mt-2 text-sm sm:text-base"
                      />
                    ))}
